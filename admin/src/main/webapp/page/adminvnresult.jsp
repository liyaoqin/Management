...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#demo'
            ,url: '${ pageContext.request.contextPath }/result/adminviewnresult'
            ,width:2500
            ,page: true //开启分页
            ,cols: [[ //表头
                ...
                ,{field: 'approve', title: '审批', width:150}
        ...
        $(document).on('click','#pass',function () {
            ...
            parent.layer.confirm('确定通过此结题成果申报吗', function(index){
                parent.layer.close(index);
                $.post("${ pageContext.request.contextPath }/result/passresult/"+pid,function(data){
                    if(data.status==200){
        ...
        $(document).on('click','#npass',function () {
            var pid=$(this).attr("pid");
            parent.layer.open({
                type: 2,
                title: '请填写不通过此结题成果的原因',
                shadeClose: false, //点击遮罩关闭
                area: ['500px', '500px'],
                content: '${ pageContext.request.contextPath }/page/npassresult/'+pid
            });
        });
    });
</script>
