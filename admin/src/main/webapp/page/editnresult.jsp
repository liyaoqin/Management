...
<form class="layui-form" action="">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="projectTitle" required  lay-verify="required" value="${result.projectTitle}" autocomplete="off"
                   class="layui-input" disabled>
        </div>
    </div>


    <input type="hidden" name="cresults" id="myinput" value="${result.cresults}">
    <label class="layui-form-label">上传结题材料</label>
    <button type="button" class="layui-btn" id="test1">
        <i class="layui-icon">&#xe67c;</i>点击上传
    </button>
    文件名称: <span id="fileName">${result.cresults}</span><br>
    <button type="button" class="layui-btn" id="test9">重新上传结题材料(请下载结题报告书填写好后上传)</button><br/>
    <p id="demoText"></p>
...
<script>
    ...
    layui.use(["layer","form","upload","jquery","element"], function(){
        ...
        form.on('submit(formDemo)', function(data){
            $.post("${ pageContext.request.contextPath }/result/editnresult", data.field,function(data){
                if (data.status == 200) {
